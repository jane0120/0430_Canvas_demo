/**
 * Created by jianyujing on 2016/4/15.
 */
...
function drawGrid(color, stepx, stepy) {//画标注线
    context.save();
...
function drawCentroid() {//画仪表中心的小圆
    context.beginPath();
    context.save();
...
function drawCentroidGuidewire(loc) {//指针上的小圆
    var angle = -Math.PI / 4,
        radius,
        endpt;
...
function drawRing() {//利用非零环绕原则 绘制剪纸效果的仪表外框（圆环）
    drawRingOuterCircle();
...
function drawRingOuterCircle() {//逆时针绘制外围
    ...
    function drawTickInnerCircle() {//顺时针绘制文字标签圆
        context.save();
        context.beginPath();
        context.strokeStyle = 'rgba(0,0,0,0.1)';
    ...
    function drawTick(angle, radius, cnt) {//绘制单个刻度。利用半径与半径与x轴夹角
    ...
    function drawAnnotations() { //绘制表盘文字
        var radius = circle.radius + RING_INNER_RADIUS;
        context.save();
